...
async function runTests() {
    ...
    if (healthRes.currency !== 'PKR')
        throw new Error('Currency is not PKR');
    ...
    if (!userLoginRes.success || userLoginRes.user.role !== 'user')
    ...
    if (forbiddenRes.status !== 403)
        throw new Error('Security flaw: User was not blocked from admin action!');
    ...
    const newProductRes = await fetch(`${API_BASE}/products`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    const stockUpdateRes = await fetch(`${API_BASE}/products/${createdProductId}/stock`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminToken}`
    ...
    if (stockUpdateRes.product?.stock !== 20)
    ...
    const newOfferRes = await fetch(`${API_BASE}/offers`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminToken}`
    ...
    const checkProd = await fetch(`${API_BASE}/products/${createdProductId}`).then(r => r.json());
    console.log('Stock after 2 units ordered (was 20):', checkProd.product?.stock);
    if (checkProd.product?.stock !== 18)
        throw new Error('Inventory was not decremented correctly!');
    ...
    const statusRes = await fetch(`${API_BASE}/orders/${orderId}/status`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminToken}`
