...
const DATA_FILE = path_1.default.join(__dirname, '../../data_backup.json');
...
class MemoryDataStore {
    ...
    init() {
        ...
        const samiAdminPassword = bcryptjs_1.default.hashSync('561703SM*Store', salt);
        const primaryAdmin = {
            _id: 'admin_samiullah',
        ...
        if (fs_1.default.existsSync(DATA_FILE)) {
            try {
                ...
                this.users = (data.users || []).filter((u) => u.name !== 'Hamza Khan' && u.email !== 'user@store.pk');
                this.products = data.products || [];
                this.offers = data.offers || [];
                this.orders = (data.orders || []).filter((o) => o.customerName !== 'Hamza Khan');
                if (data.settings) {
    ...
    async getSettings() {
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                const dbSettings = await Settings_1.SettingsModel.findOne().lean();
                if (dbSettings) {
                    this.settings = {
    ...
    async updateSettings(newSettings) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await Settings_1.SettingsModel.findOneAndUpdate({}, this.settings, { upsert: true, new: true });
            }
            catch (err) {
    ...
    async findUserByEmail(email) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                const dbUser = await User_1.UserModel.findOne({ email: cleanEmail }).lean();
                if (dbUser)
                    user = dbUser;
            }
            catch (err) {
    ...
    async findUserById(id) {
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                const dbUser = await User_1.UserModel.findById(id).lean();
                if (dbUser)
                    return dbUser;
            }
            catch (err) {
    ...
    async createUser(userData) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await User_1.UserModel.findOneAndUpdate({ email: userData.email }, userData, { upsert: true, new: true });
            }
            catch (err) {
    ...
    async updateUserPassword(email, hashedPassword) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await User_1.UserModel.updateOne({ email: cleanEmail }, { $set: { password: hashedPassword, ...(isAdmin ? { role: 'admin' } : {}) } });
            }
            catch (err) {
    ...
    async saveOtp(email, otp) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await Otp_1.OtpModel.findOneAndUpdate({ email: cleanEmail }, { email: cleanEmail, otp, expiresAt: new Date(expiresAt) }, { upsert: true, new: true });
            }
            catch (err) {
                // fallback
            }
    ...
    async verifyOtp(email, otp) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                const dbOtp = await Otp_1.OtpModel.findOne({ email: cleanEmail, otp: cleanOtp });
    ...
    async clearOtp(email) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await Otp_1.OtpModel.deleteMany({ email: cleanEmail });
            }
            catch (err) {
                // fallback
            }
...
exports.store = new MemoryDataStore();
